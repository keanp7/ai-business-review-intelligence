...
export async function handle(request: Request) {
  try {
    ...
    // Trigger auto-matching of businesses (fire-and-forget)
    Promise.resolve().then(async () => {
      try {
        const matchResult = await autoMatchBusinesses(newUser.id, email);
        console.log(
          `Auto-matched ${matchResult.matchedCount} businesses for user ${newUser.id}`
        );
      } catch (err) {
        console.error("Error auto-matching businesses:", err);
      }
    });
